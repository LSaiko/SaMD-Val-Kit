...
class FDACybersecurityChecker:
    ...
    def _build_checklist(self):
        ...
        self._add(
            CyberCheckItem(
                ...
                references=[
                    "FDA Postmarket Cyber 2016 §IV.D",
                    "NIST SP 800-61",
                    "FDA MDR 21 CFR 803",
                    "H-ISAC",
                ],
        ...
        if self.sbom is None:
            for c in self.controls:
                if c.control_id == "CY-TR-001":
                    ...
                    c.recommendation = (
                        "Generate SBOM using SBOMGenerator and pass to checker."
                    )
    ...
    def non_compliant_count(self) -> int:
        return sum(
            1 for c in self.controls if c.status == CyberControlStatus.NON_COMPLIANT
        )
    ...
    def summary(self) -> Dict:
        return {
            ...
            "Not Assessed": sum(
                1 for c in self.controls if c.status == CyberControlStatus.NOT_ASSESSED
            ),
        }
    ...
    def print_summary(self):
        ...
        for c in self.controls:
            ...
            print(
                f"  {c.control_id:<14} {c.domain:<28} {mandatory:<7} {c.status.value}"
            )
        print()
