...
class SBOMComponent:
    ...
    hash_value: str = ""
    component_type: str = (
        "library"  # library | framework | application | container | os
    )
    language: str = ""
    homepage: str = ""
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "Hash": (
                f"{self.hash_algorithm}:{self.hash_value}"
                if self.hash_value
                else "Not provided"
            ),
        }
...
class SBOMGenerator:
    ...
    def export_spdx_json(self) -> Dict:
        ...
        return {
            ...
            "creationInfo": {
                ...
                "creators": [
                    "Tool: samd-validation-toolkit",
                    f"Organization: {self.device.manufacturer}",
                ],
            },
            "packages": [
                {
    ...
    def summary(self) -> Dict:
        return {
            ...
            "Direct Dependencies": sum(
                1 for c in self.components if c.dependency_relationship == "Direct"
            ),
            "Components with Known CVEs": len(self.components_with_vulnerabilities()),
            "License Types": len(
                set(c.license_id for c in self.components if c.license_id)
            ),
    ...
    def print_summary(self):
        ...
        for c in self.components:
            cves = (
                ", ".join(c.known_vulnerabilities)
                if c.known_vulnerabilities
                else "None"
            )
