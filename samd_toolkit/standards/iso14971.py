...
class RiskManagementFile:
        ...
        if self.device.device_class in (DeviceClass.CLASS_II, DeviceClass.CLASS_III):
            self.add_risk(
                RiskItem(
                    ...
                    hazardous_situation="Device reports 'normal' when patient has critical finding",
    ...
    def unacceptable_risks(self) -> List[RiskItem]:
        return [
            r
            for r in self.risks
            if r.residual_acceptability == RiskAcceptability.UNACCEPTABLE
        ]

    @property
    def alarp_risks(self) -> List[RiskItem]:
        return [
            r for r in self.risks if r.residual_acceptability == RiskAcceptability.ALARP
        ]
    ...
    def summary(self) -> Dict:
        return {
            ...
            "Overall Residual Risk": (
                "ACCEPTABLE"
                if self.overall_residual_risk_acceptable
                else "UNACCEPTABLE — ACTION REQUIRED"
            ),
        }
    ...
    def print_summary(self):
        ...
        for r in self.risks:
            print(
                f"  {r.risk_id:<12} {r.initial_acceptability.value:<10} {r.residual_acceptability.value:<12} {r.hazard[:45]}"
            )
        print()
